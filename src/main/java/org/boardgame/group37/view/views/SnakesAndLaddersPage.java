...
import org.boardgame.group37.view.*;
    ...
    public static void init(Pane root, MainController mainController){
        ...
        for (int i = 0; i < filenames.length; i++){
            ...
            filebutton.setOnAction(e -> {
                mainController.fileButton(filename, root);
            });
        ...
        newBoardButton.setOnAction(e -> {
            mainController.initCraftingBoard(root, mainController);
        });
        ...
        choiceBox.getSelectionModel().selectedIndexProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue ov, Number value, Number new_value)
            {
                ...
                for (int i = mainController.getPlayerController().getNumberOfPlayers(); i < (new_value.intValue() + 2); i++) {
                    ...
                    mainController.getPlayerController().addPlayer("Player " + (i + 1), ColorPalette.getPlayerColors()[i]);
                }
                for (int i = mainController.getPlayerController().getNumberOfPlayers(); i > (new_value.intValue() + 2); i--) {
